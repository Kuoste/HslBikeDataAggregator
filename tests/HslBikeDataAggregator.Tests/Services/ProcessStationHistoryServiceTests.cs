...
public sealed class ProcessStationHistoryServiceTests
{
    ...
    public async Task ProcessAsync_UsesOnlyNewestTwoAvailableMonths()
    {
        ...
        var station001Destinations = Assert.IsAssignableFrom<IReadOnlyList<StationHistory>>(writtenDestinations["smoove:001"]);
        Assert.Collection(
            ...
            destination =>
            {
                Assert.Equal("smoove:003", destination.ArrivalStationId);
                Assert.Equal(1, destination.TripCount);
            });
    ...
    public async Task ProcessAsync_SearchesBackwardsAndDeletesStaleDestinationProfiles()
    {
        ...
        Assert.Equal("smoove:002", station001Destination.ArrivalStationId);
        ...
        var station002Destinations = Assert.IsAssignableFrom<IReadOnlyList<StationHistory>>(writtenDestinations["smoove:002"]);
        var station002Destination = Assert.Single(station002Destinations);
        Assert.Equal("smoove:001", station002Destination.ArrivalStationId);
    ...
    [Fact]
    public async Task ProcessAsync_PreservesCanonicalStationIdsFromSource()
    {
        var handler = new StubHttpMessageHandler(request =>
        {
            var url = request.RequestUri?.ToString();
            ...
            return Task.FromResult(url switch
            {
                "https://example.test/2024-07.csv" => CreateResponse(
                    ...
                    """
                    Departure station id,Return station id,Covered distance (m),Duration (sec.)
                    smoove:001,smoove:002,1200,300
                    """),
                _ => CreateResponse(HttpStatusCode.NotFound)
            });
        });
        ...
        var stationDestinations = Assert.IsAssignableFrom<IReadOnlyList<StationHistory>>(writtenDestinations["smoove:001"]);
        var stationDestination = Assert.Single(stationDestinations);
        Assert.Equal("smoove:002", stationDestination.ArrivalStationId);
    }
